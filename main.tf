...
resource "null_resource" "update_nodes_env" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "docker-compose up -d",
      "sleep 15s",
      "docker exec mysql mysql -h 172.17.0.1 --port ${each.value.database_admin.port} -u ${each.value.database_admin.user} -p${each.value.database_admin.password} -e \"stop slave;\"",
      "docker exec mysql mysql -h 172.17.0.1 --port ${each.value.database_admin.port} -u ${each.value.database_admin.user} -p${each.value.database_admin.password} -e \"CHANGE MASTER TO MASTER_HOST='${elestio_mysql.nodes[[for n in elestio_mysql.nodes : n.server_name if n.server_name != each.value.server_name][0]].global_ip}', MASTER_PORT=${elestio_mysql.nodes[[for n in elestio_mysql.nodes : n.server_name if n.server_name != each.value.server_name][0]].database_admin.port}, MASTER_USER='${elestio_mysql.nodes[[for n in elestio_mysql.nodes : n.server_name if n.server_name != each.value.server_name][0]].database_admin.user}', MASTER_PASSWORD='${elestio_mysql.nodes[[for n in elestio_mysql.nodes : n.server_name if n.server_name != each.value.server_name][0]].database_admin.password}', MASTER_AUTO_POSITION=1;\"",
      "docker exec mysql mysql -h 172.17.0.1 --port ${each.value.database_admin.port} -u ${each.value.database_admin.user} -p${each.value.database_admin.password} -e \"reset slave;\"",
      "docker exec mysql mysql -h 172.17.0.1 --port ${each.value.database_admin.port} -u ${each.value.database_admin.user} -p${each.value.database_admin.password} -e \"start slave;\"",
      "docker exec mysql mysql -h 172.17.0.1 --port ${each.value.database_admin.port} -u ${each.value.database_admin.user} -p${each.value.database_admin.password} -e \"show slave status;\"",
    ]
  }
}
